...
void EnergyFunctional::setAdjointsF(CalibHessian* Hcalib)
{
	...
	for(int h=0;h<nFrames;h++)
		for(int t=0;t<nFrames;t++)
		{
			...
			//std::cout << "0: hsetAdjointsF(): target->get_worldToCam_evalPT().Adj()=\n" << target->get_worldToCam_evalPT().Adj() << "\n";
			//std::cout << "0: hsetAdjointsF(): host->get_worldToCam_evalPT().Adj()=\n" << host->get_worldToCam_evalPT().Adj() << "\n";
			//std::cout << "0: hsetAdjointsF(): hostToTarget.Adj().transpose()=\n" << hostToTarget.Adj().transpose() << "\n";
			...
			//std::cout << "0: hsetAdjointsF(): affLL=\n" << affLL << "\n";
			AT(6,6) = -affLL[0];
			AH(6,6) = affLL[0];
			AT(7,7) = -1;
			AH(7,7) = affLL[0];
			...
			//std::cout << "0: hsetAdjointsF(): h=" << h << "t=" << t << "AH=\n" << AH << "\n";
		}
...
void EnergyFunctional::dropResidual(EFResidual* r)
{
	...
	// cinetec mod: by Arthur
	/* initial code
    connectivityMap[(((uint64_t)r->host->frameID) << 32) + ((uint64_t)r->target->frameID)][0]--;
	nResiduals--;*/
	const uint64_t key = (((uint64_t)r->host->frameID) << 32) + ((uint64_t)r->target->frameID);
	if (connectivityMap.find(key) != connectivityMap.end())
	{
		connectivityMap[key][0]--;
		nResiduals--;
	}
	/////////////////////////
	r->data->efResidual=0;
...
void EnergyFunctional::solveSystemF(int iteration, double lambda, CalibHessian* HCalib)
{
	...
	else
	{
		...
		//std::cout << "3.2: solveSystemF(): HL_top=\n" << HL_top << "\n";
		//std::cout << "3.2: solveSystemF(): HM=\n" << HM << "\n";
		//std::cout << "3.2: solveSystemF(): HA_top=\n" << HA_top << "\n";
	...
	else
	{
		...
		//std::cout << "4.2: solveSystemF(): HFinal_top=\n" << HFinal_top << "\n";
		//std::cout << "4.2: solveSystemF(): HFinal_top.diagonal()=\n" << HFinal_top.diagonal() << "\n";
		//std::cout << "4.2: solveSystemF(): VecX::Constant(HFinal_top.cols(), 10)=\n" << VecX::Constant(HFinal_top.cols(), 10) << "\n";
		//std::cout << "4.2: solveSystemF(): (HFinal_top.diagonal()+VecX::Constant(HFinal_top.cols(), 10)).cwiseSqrt()=\n" << (HFinal_top.diagonal() + VecX::Constant(HFinal_top.cols(), 10)).cwiseSqrt() << "\n";
		//std::cout << "4.2: solveSystemF(): SVecI=\n" << SVecI << "\n";
		//std::cout << "4.2: solveSystemF(): HFinalScaled=\n" << HFinalScaled << "\n";
		//std::cout << "4.2: solveSystemF(): bFinal_top=\n" << bFinal_top << "\n";
		//std::cout << "4.2: solveSystemF(): x=" << x << "\n";
	}
