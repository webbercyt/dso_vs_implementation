...
namespace dso
{
ImmaturePoint::ImmaturePoint(int channel, int u_, int v_, FrameHessian* host_, float type, CalibHessian* HCalib)
: u(u_), v(v_), host(host_), my_type(type), idepth_min(0), idepth_max(NAN), lastTraceStatus(IPS_UNINITIALIZED), channel(channel)
{
	...
	for(int idx=0;idx<patternNum;idx++)
	{
        ...
        Vec3f ptc = getInterpolatedElement33BiLin(host->dI, u+dx, v+dy, globalCalibs[channel].wG[0], globalCalibs[channel].hG[0]);
 ...
 */
ImmaturePointStatus ImmaturePoint::traceOn(const FrameHessian* const frame, const Mat33f &hostToFrame_KRKi, const Vec3f &hostToFrame_Kt, const Vec2f &hostToFrame_affine, const CalibHessian* HCalib, bool debugPrint)
{
	if(lastTraceStatus == ImmaturePointStatus::IPS_OOB) return lastTraceStatus;


	debugPrint = false;//rand()%100==0;
	float maxPixSearch = (globalCalibs[channel].wG[0]+ globalCalibs[channel].hG[0])*setting_maxPixSearch;
...
//	const float minImprovementFactor = 2;		// if pixel-interval is smaller than this, leave it be.
	...
	if(!(uMin > 4 && vMin > 4 && uMin < globalCalibs[channel].wG[0]-5 && vMin < globalCalibs[channel].hG[0]-5))
	{
	...
	if(std::isfinite(idepth_max))
	{
		...
		if(!(uMax > 4 && vMax > 4 && uMax < globalCalibs[channel].wG[0]-5 && vMax < globalCalibs[channel].hG[0]-5))
		{
	...
	for(int i=0;i<numSteps;i++)
	{
		...
		for(int idx=0;idx<patternNum;idx++)
		{
			float hitColor = getInterpolatedElement31(frame->dI,
										...
										globalCalibs[channel].wG[0], globalCalibs[channel].hG[0]);
	...
	for(int it=0;it<setting_trace_GNIterations;it++)
	{
		...
		for(int idx=0;idx<patternNum;idx++)
		{
			Vec3f hitColor = getInterpolatedElement33(frame->dI,
					...
					(float)(bestV+rotatetPattern[idx][1]), globalCalibs[channel].wG[0], globalCalibs[channel].hG[0]);
...
//	float absGrad0 = getInterpolatedElement(frame->absSquaredGrad[0],bestU, bestV, wG[0], hG[0]);
//	float absGrad1 = getInterpolatedElement(frame->absSquaredGrad[1],bestU*0.5-0.25, bestV*0.5-0.25, wG[1], hG[1]);
//	float absGrad2 = getInterpolatedElement(frame->absSquaredGrad[2],bestU*0.25-0.375, bestV*0.25-0.375, wG[2], hG[2]);
		...
		float idepth)
{
	...
	projectPoint(channel, this->u,this->v, idepth, 0, 0,HCalib,
		...
		float idepth)
{
	...
	float energyLeft=0;
	const Eigen::Vector3f* const dIl = tmpRes->target->dI;
	...
	{
		float Ku, Kv;
		if(!projectPoint(this->channel, this->u+patternP[idx][0], this->v+patternP[idx][1], idepth, PRE_KRKiTll, PRE_KtTll, Ku, Kv))
			{return 1e10;}

		Vec3f hitColor = (getInterpolatedElement33(dIl, Ku, Kv, globalCalibs[channel].wG[0], globalCalibs[channel].hG[0]));
		if(!std::isfinite((float)hitColor[0])) {return 1e10;}
		//if(benchmarkSpecialOption==5) hitColor = (getInterpolatedElement13BiCub(tmpRes->target->I, Ku, Kv, wG[0], hG[0]));
...
double ImmaturePoint::linearizeResidual(
		...
		float idepth)
{
	if(tmpRes->state_state == ResState::ResState_OOB)
		{ tmpRes->state_NewState = ResState::ResState_OOB; return tmpRes->state_energy; }
	...
	for(int idx=0;idx<patternNum;idx++)
	{
		...
		if(!projectPoint(this->channel, this->u,this->v, idepth, dx, dy,HCalib,
		...
		if(!std::isfinite((float)hitColor[0])) {tmpRes->state_NewState = ResState::ResState_OOB; return tmpRes->state_energy;}
		float residual = hitColor[0] - (affLL[0] * color[idx] + affLL[1]);
	...
	{
		energyLeft = energyTH*outlierTHSlack;
		tmpRes->state_NewState = ResState::ResState_OUTLIER;
	}
	else
	{
		tmpRes->state_NewState = ResState::ResState_IN;
	}
