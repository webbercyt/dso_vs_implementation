...
namespace dso
{
	...
	void FullSystem::debugPlotTracking()
	{
		...
		int wh = globalCalibs[channel].hG[0]* globalCalibs[channel].wG[0];
		...
		for(FrameHessian* f : frameHessians)
		{
			...
			for(FrameHessian* f2 : frameHessians)
				if(f2->debugImage == 0) f2->debugImage = new MinimalImageB3(globalCalibs[channel].wG[0], globalCalibs[channel].hG[0]);
			...
			for(FrameHessian* f2 : frameHessians)
			{
				...
				const Eigen::Vector3f* const fd = f2->dI;
	...
	void FullSystem::debugPlot(std::string name)
	{
		...
		for(unsigned int f=0;f<frameHessians.size();f++)
		{
			MinimalImageB3* img = new MinimalImageB3(globalCalibs[channel].wG[0], globalCalibs[channel].hG[0]);
			images.push_back(img);
			//float* fd = frameHessians[f]->I;
			const Eigen::Vector3f* const fd = frameHessians[f]->dI;
