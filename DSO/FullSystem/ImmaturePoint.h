...
public:
	...
	float idepth_min;
	float idepth_max;
	ImmaturePoint(int channel, int u_, int v_, FrameHessian* host_, float type, CalibHessian* HCalib);
	~ImmaturePoint();

	ImmaturePointStatus traceOn(const FrameHessian* const frame, const Mat33f &hostToFrame_KRKi, const Vec3f &hostToFrame_Kt, const Vec2f &hostToFrame_affine, const CalibHessian* HCalib, bool debugPrint=false);
