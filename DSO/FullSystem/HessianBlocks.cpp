...
void FrameHessian::makeImages(float* color, CalibHessian* HCalib)
{
	...
	{
		dIp[i] = new Eigen::Vector3f[globalCalibs[channel].wG[i]* globalCalibs[channel].hG[i]];
		absSquaredGrad[i] = new float[globalCalibs[channel].wG[i]* globalCalibs[channel].hG[i]];
	}
	dI = dIp[0];

	// make d0
	int w = globalCalibs[channel].wG[0];
	int h = globalCalibs[channel].hG[0];
	for (int i = 0; i<w*h; i++)
		dI[i][0] = color[i];

	for (int lvl = 0; lvl<pyrLevelsUsed; lvl++)
	{
		int wl = globalCalibs[channel].wG[lvl], hl = globalCalibs[channel].hG[lvl];
		...
		{
			int lvlm1 = lvl - 1;
			int wlm1 = globalCalibs[channel].wG[lvlm1];
			int hlm1 = globalCalibs[channel].hG[lvlm1];
...
cv::Mat FrameHessian::getImage()
{
	std::vector<cv::Mat> spl;
	cv::split(cv::Mat(globalCalibs[channel].hG[0], globalCalibs[channel].wG[0], CV_32FC3, dI), spl);
	cv::Mat ret;
	spl[0].convertTo(ret, CV_8UC1);
