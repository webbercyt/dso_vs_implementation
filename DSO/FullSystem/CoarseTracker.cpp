...
CoarseTracker::CoarseTracker(int channel, int ww, int hh) : 
	channel(channel),
	lastRef_aff_g2l(0,0)
{
...
{
	w[0] = globalCalibs[channel].wG[0];
	h[0] = globalCalibs[channel].hG[0];
...
void CoarseTracker::makeCoarseDepthL0_Reductor0(FrameHessian* fh, int min, int max, Vec10* stats, int tid)
{
	for (int i = min; i < max; i++)
	{
		PointHessian* ph = fh->pointHessians[i];
		if (ph->lastResiduals[0].first != 0 && ph->lastResiduals[0].second == ResState::ResState_IN)
		{
...
void CoarseTracker::makeCoarseDepthL0_Reductor1(int lvl, int min, int max, Vec10* stats, int tid)
{
	int lvlm1 = lvl - 1;
...
void CoarseTracker::makeCoarseDepthL0_Reductor2(int lvl, float* weightSumsl_bak, int min, int max, Vec10* stats, int tid)
{
	int wl = w[lvl];
	int hl = h[lvl];
	int wh = wl * hl;
...
void CoarseTracker::makeCoarseDepthL0_Reductor3(int lvl, float* weightSumsl_bak, int min, int max, Vec10* stats, int tid)
{
	int wl = w[lvl];
	int hl = h[lvl];
	int wh = wl * hl;
...
void CoarseTracker::makeCoarseDepthL0_Reductor4(int lvl, makeCoarseDepthL0Result *result, int min, int max, Vec10* stats, int tid)
{
	float* weightSumsl = weightSums[lvl];
	float* idepthl = idepth[lvl];
	const Eigen::Vector3f* const dIRefl = lastRef->dIp[lvl];
	...
	{
		int x = i % wl;
		if (x < 2 || x >= wl - 2) continue;

		int y = i / wl;
		if (y < 2 || y >= hl - 2) continue;

		if (weightSumsl[i] > 0)
		{
			idepthl[i] /= weightSumsl[i];
			if (!std::isfinite(dIRefl[i][0]) || !(idepthl[i]>0))
			{
				idepthl[i] = -1;
				continue;	// just skip if something is wrong.
			}

			result->lpc_u[tid].push_back(float(x));
			result->lpc_v[tid].push_back(float(y));
			result->lpc_idepth[tid].push_back(idepthl[i]);
			result->lpc_color[tid].push_back(dIRefl[i][0]);
			result->lpc_n[tid]++;
		}
		else
			idepthl[i] = -1;
...
void CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians)
{
	...
	for(FrameHessian* fh : frameHessians)
	{
		if (multiThreading)
		{
			makeCoarseDepthL0Reduce.reduce(boost::bind(&CoarseTracker::makeCoarseDepthL0_Reductor0, this, fh, _1, _2, _3, _4), 0, fh->pointHessians.size(), 0);
		}
		else
		{
	...
	for(int lvl=1; lvl<pyrLevelsUsed; lvl++)
	{
		if (multiThreading)
		{
			makeCoarseDepthL0Reduce.reduce(boost::bind(&CoarseTracker::makeCoarseDepthL0_Reductor1, this, lvl, _1, _2, _3, _4), 0, w[lvl] * h[lvl], 0);
		}
		else
		{
			int lvlm1 = lvl - 1;
	...
	for(int lvl=0; lvl<2; lvl++)
	{
		...
		for(int it=0;it<numIts;it++)
		{
			if (multiThreading)
			{
				...
				memcpy(weightSumsl_bak, weightSums[lvl], w[lvl] * h[lvl] * sizeof(float));
				makeCoarseDepthL0Reduce.reduce(boost::bind(&CoarseTracker::makeCoarseDepthL0_Reductor2, this, lvl, weightSumsl_bak, _1, _2, _3, _4), 0, w[lvl] * h[lvl], 0);
			}
			else
			{
	...
	for(int lvl=2; lvl<pyrLevelsUsed; lvl++)
	{
		if (multiThreading)
		{
			float* weightSumsl_bak = weightSums_bak[lvl];
			memcpy(weightSumsl_bak, weightSums[lvl], w[lvl] * h[lvl] * sizeof(float));
			makeCoarseDepthL0Reduce.reduce(boost::bind(&CoarseTracker::makeCoarseDepthL0_Reductor3, this, lvl, weightSumsl_bak, _1, _2, _3, _4), 0, w[lvl] * h[lvl], 0);
		}
		else
		{
			int wh = w[lvl] * h[lvl] - w[lvl];
	...
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		{
			result.lpc_n[i] = 0;
			result.lpc_u[i].clear();
			result.lpc_v[i].clear();
			result.lpc_idepth[i].clear();
			result.lpc_color[i].clear();
		}

		if (multiThreading)
		{
			makeCoarseDepthL0Reduce.reduce(boost::bind(&CoarseTracker::makeCoarseDepthL0_Reductor4, this, lvl, &result, _1, _2, _3, _4), 0, w[lvl] * h[lvl], 0);

		}
		else
		{
			makeCoarseDepthL0_Reductor4(lvl, &result, 0, w[lvl] * h[lvl], 0, 0);
		...
		for (int i = 0; i < NUM_THREADS; i++)
		{
			for (int j = 0; j < result.lpc_n[i]; j++)
			{
				lpc_u[lpc_n] = result.lpc_u[i][j];
				lpc_v[lpc_n] = result.lpc_v[i][j];
				lpc_idepth[lpc_n] = result.lpc_idepth[i][j];
				lpc_color[lpc_n] = result.lpc_color[i][j];
				lpc_n++;
			}
		}
		pc_n[lvl] = lpc_n;
...
Vec6 CoarseTracker::calcRes(int lvl, const SE3 &refToNew, AffLight aff_g2l, float cutoffTH)
{
	...
	int wl = w[lvl];
	int hl = h[lvl];
	const Eigen::Vector3f* const dINewl = newFrame->dIp[lvl];
	float fxl = fx[lvl];
	float fyl = fy[lvl];
	float cxl = cx[lvl];
	float cyl = cy[lvl];
	...
	int nl = pc_n[lvl];
	const float* const lpc_u = pc_u[lvl];
	const float* const lpc_v = pc_v[lvl];
	const float* const lpc_idepth = pc_idepth[lvl];
	const float* const lpc_color = pc_color[lvl];
...
bool CoarseTracker::trackNewestCoarse(
		...
		int coarsestLvl,
		Vec6 minResForAbort,	//mod by Arthur: 5-->6
		IOWrap::Output3DWrapper* wrap)
{
	debugPlot = setting_render_displayCoarseTrackingFull;
	debugPrint = false;

	assert(coarsestLvl < 6 && coarsestLvl < pyrLevelsUsed);	//mod by Arthur: 5-->6

	lastResiduals.setConstant(NAN);
	lastFlowIndicators.setConstant(1000);


	newFrame = (FrameHessian*)newFrameHessian;
	...
	for(int lvl=coarsestLvl; lvl>=0; lvl--)
	{
		...
		if(levelCutoffRepeat > 1 && !haveRepeated)
		{
			lvl++;
			haveRepeated=true;
			if (debugPrint) printf("REPEAT LEVEL!\n");
		}
	}
...
void CoarseTracker::depth_Reductor(int lvl, MinimalImageB3 *mf, float minID, float maxID, int min, int max, Vec10* stats, int tid)
{
	const Eigen::Vector3f* const dIpl = lastRef->dIp[lvl];
	for (int idx = min; idx < max; idx++)
	{
		int c = dIpl[idx][0] * 0.9f;
		if (c > 255) c = 255;
		mf->at(idx) = Vec3b(c, c, c);
	}
	...
	for (int idx = min; idx < max; idx++)
	{
		int wl = idx % w[lvl];
		int hl = idx / w[lvl];
		if (wl < 3 || wl >= w[lvl] - 3) continue;
		if (hl < 3 || hl >= h[lvl] - 3) continue;
		...
		if (bp[0] > 0 || nid >= 3)
		{
			...
			mf->setPixelCirc(wl, hl, makeJet3B(id));
		}
	}
}
...
void CoarseTracker::debugPlotIDepthMap(float* minID_pt, float* maxID_pt, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
...

	{
		...
		if (allID.size() == 0) return;// add by Arthur
		...
		if (multiThreading)
		{
			depthReduce.reduce(boost::bind(&CoarseTracker::depth_Reductor, this, lvl, &mf, minID, maxID,
				_1, _2, _3, _4), 1, w[lvl]*h[lvl], 0);
		}
		else
		{
			for (int i = 0; i < h[lvl] * w[lvl]; i++)
		...
		if (setting_useProfile[channel]) printf("\t\t\t\tdebugPlotIDepthMap():get depth image: %f ms\n", duration * 1000.0f);
...
void CoarseTracker::clone(CoarseTracker* src)
{
	lastRef = src->lastRef;
	lastRef_aff_g2l = src->lastRef_aff_g2l;
	newFrame = src->newFrame;
	refFrameID = src->refFrameID;

	lastResiduals = src->lastResiduals;
	lastFlowIndicators = src->lastFlowIndicators;
	firstCoarseRMSE = src->firstCoarseRMSE;

	for (int lvl = 0; lvl < PYR_LEVELS; lvl++)
	{
		idepth[lvl] = src->idepth[lvl];
		weightSums[lvl] = src->weightSums[lvl];
		weightSums_bak[lvl] = src->weightSums_bak[lvl];

		pc_u[lvl] = src->pc_u[lvl];
		pc_v[lvl] = src->pc_v[lvl];
		pc_idepth[lvl] = src->pc_idepth[lvl];
		pc_color[lvl] = src->pc_color[lvl];
	}

	memcpy(pc_n, src->pc_n, PYR_LEVELS * sizeof(int));
...
CoarseDistanceMap::CoarseDistanceMap(int channel, int ww, int hh) :
	channel(channel)
{
