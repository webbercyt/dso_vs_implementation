...
void FullSystem::linearizeAll_Reductor(bool fixLinearization, std::vector<PointFrameResidual*>* toRemove, int min, int max, Vec10* stats, int tid)
{
	...
	for (int k = min; k < max; k++)
	{
		...
		float result = r->linearize(Hcalib);
		(*stats)[0] += result;
	...
	if (setting_useProfile[channel]) {
		...
		printf("\t\t\t\t\t\t\tFullSystem::linearizeAll_Reductor(): min=%d  max=%d  %f ms\n", min, max, duration * 1000.0f);
	}
}
...
Vec3 FullSystem::linearizeAll(bool fixLinearization)
{
	...
	if(multiThreading)
	{
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		if (setting_useProfile[channel]) printf("\t\t\t\t\t\tFullSystem::linearizeAll(): treadReduce.reduce: %f ms\n", duration * 1000.0f);
	}
	else
	{
		start = clock();
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		if (setting_useProfile[channel]) printf("\t\t\t\t\t\tFullSystem::linearizeAll(): linearizeAll_Reductor: %f ms\n", duration * 1000.0f);
	}
	...
	duration = (double)(end - start) / CLOCKS_PER_SEC;
	if (setting_useProfile[channel]) printf("\t\t\t\t\t\tFullSystem::linearizeAll(): setNewFrameEnergyTH: %f ms\n", duration * 1000.0f);
	...
	if(fixLinearization)
	{
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		if (setting_useProfile[channel]) printf("\t\t\t\t\t\tFullSystem::linearizeAll(): for(int i=0;i<NUM_THREADS;i++): %f ms\n", duration * 1000.0f);
...
//	meanStepC += Hcalib->step.norm();
	...
	else
	{
		...
		for(FrameHessian* fh : frameHessians)
		{
			if (!std::isfinite(fh->step[6]) || !std::isfinite(fh->step[7]) || !std::isfinite(fh->step[8]) || !std::isfinite(fh->step[9])) return false;
...
float FullSystem::optimize(int mnumOptIts)
{
	...
	if (setting_useProfile[channel]) {
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		printf("\t\t\t\tFullSystem::optimize():get statistics and active residuals: %f ms\n", duration * 1000.0f);
	}
	...
	if (setting_useProfile[channel]) {
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		printf("\t\t\t\tFullSystem::optimize(): linearizeAll: %f ms\n", duration * 1000.0f);
		start = clock();
	}
	...
	if (setting_useProfile[channel]) {
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		printf("\t\t\t\tFullSystem::optimize(): calcLEnergy: %f ms\n", duration * 1000.0f);
		start = clock();
	}
	...
	if (setting_useProfile[channel]) {
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		printf("\t\t\t\tFullSystem::optimize(): calcMEnergy: %f ms\n", duration * 1000.0f);
	}
	...
	if (setting_useProfile[channel]) {
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		printf("\t\t\t\tFullSystem::optimize(): treadReduce: %f ms\n", duration * 1000.0f);
	}
	...
	if (setting_useProfile[channel]) {
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		printf("\t\t\t\tFullSystem::optimize(): debugPlotTracking: %f ms\n", duration * 1000.0f);
	}
	...
	for(int iteration=0;iteration<mnumOptIts;iteration++)
	{
		...
		if (setting_useProfile[channel]) {
			...
			duration = (double)(end - start) / CLOCKS_PER_SEC;
			printf("\t\t\t\t\tFullSystem::optimize(): backupState(): iteration:%d  %f ms\n", iteration, duration * 1000.0f);
		}
		...
		if (setting_useProfile[channel]) {
			...
			duration = (double)(end - start) / CLOCKS_PER_SEC;
			printf("\t\t\t\t\tFullSystem::optimize(): solveSystem(): iteration:%d  %f ms\n", iteration, duration * 1000.0f);
		}
		...
		if (setting_useProfile[channel]) {
			...
			duration = (double)(end - start) / CLOCKS_PER_SEC;
			printf("\t\t\t\t\tFullSystem::optimize(): doStepFromBackup(): iteration:%d  %f ms\n", iteration, duration * 1000.0f);
		}
		...
		if (setting_useProfile[channel]) {
			...
			duration = (double)(end - start) / CLOCKS_PER_SEC;
			printf("\t\t\t\t\tFullSystem::optimize(): linearizeAll: iteration:%d  %f ms\n", iteration, duration * 1000.0f);
		}
		...
		if (setting_useProfile[channel]) {
			...
			duration = (double)(end - start) / CLOCKS_PER_SEC;
			printf("\t\t\t\t\tFullSystem::optimize(): calcLEnergy: iteration:%d  %f ms\n", iteration, duration * 1000.0f);
		}
		...
		if (setting_useProfile[channel]) {
			...
			duration = (double)(end - start) / CLOCKS_PER_SEC;
			printf("\t\t\t\t\tFullSystem::optimize(): calcMEnergy: iteration:%d  %f ms\n", iteration, duration * 1000.0f);
		}
				...
				lastEnergy[0] + lastEnergy[1] + lastEnergyL + lastEnergyM))
		{
			...
			if (setting_useProfile[channel]) {
				...
				duration = (double)(end - start) / CLOCKS_PER_SEC;
				printf("\t\t\t\t\tFullSystem::optimize(): treadReduce(): iteration:%d  %f ms\n", iteration, duration * 1000.0f);
			}
		...
		else
		{
			...
			if (setting_useProfile[channel]) {
				...
				duration = (double)(end - start) / CLOCKS_PER_SEC;
				printf("\t\t\t\t\tFullSystem::optimize(): loadSateBackup/linearizeAll/calcLEnergy/calcMEnergy(): iteration:%d  %f ms\n", iteration, duration * 1000.0f);
			}
		}
	...
	if (setting_useProfile[channel]) {
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		printf("\t\t\t\tFullSystem::optimize(): lambda: %f ms\n", duration * 1000.0f);
	}
	...
	if (setting_useProfile[channel]) {
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		printf("\t\t\t\tFullSystem::optimize(): setAdjointsF: %f ms\n", duration * 1000.0f);
	}
