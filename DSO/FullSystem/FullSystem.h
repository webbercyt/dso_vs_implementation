...
template<typename T> inline void deleteOutOrder(std::vector<T*> &v, const T* element, bool deletable)
{
	int i=-1;
	for(unsigned int k=0; k<v.size();k++)
	{
...
public:
	...
	FullSystem(int channel);
	virtual ~FullSystem();

	// adds a new frame, and creates point & residual structs.
	void addActiveFrame(ImageAndExposure* image, int id);
	FrameHessian* makeImages(ImageAndExposure* image);
	void addActiveFrame(FrameHessian* fh, int id);

	// marginalizes a frame. drops / marginalizes points & residuals.
	void marginalizeFrame(FrameHessian* frame, bool deletable);
	...
	bool needToMakeKF;
	char getTrackingState();
	//void setTrackingState(char state) { fullSystemTrackingState = state; };
	bool isLostTrackerMatching = false;
	bool isCameraCovered = false;
private:
	int channel;

	CalibHessian* Hcalib;
	GroundCalibrator* gCalibrator;
	...
	Vec4 trackNewCoarse(FrameHessian* fh, bool trackBackup);
	...
	Vec6 lastCoarseRMSE;	//mod by Arthur: 5-->6
...
#define FRAMES_HISTORY_MAX 10000
#define KEYFRAMES_HISTORY_MAX 1000
	int sinceLastKF = 0; // count number of Frames since last key frame created
	int frameHistoryID = 0; // allFrameHistory.size() for use as id
	int keyFrameHistoryID = 0; // allKeyFrameHistory.size() for use as id

	struct tryResult
	{
		CoarseTracker* coarseTrackers[NUM_THREADS];
		Vec3 flowVecs[NUM_THREADS];
		SE3 lastF_2_fh[NUM_THREADS];
		AffLight aff_g2l[NUM_THREADS];
		Vec6 achievedRes[NUM_THREADS];
		bool haveOneGood[NUM_THREADS];
		int tryIterations[NUM_THREADS];
	} result;
	boost::mutex tryMutex;
	IndexThreadReduce<Vec10> tryReduce;
	void try_Reductor(FrameHessian* fh, AffLight aff_last_2_l, std::vector<SE3, Eigen::aligned_allocator<SE3>> lastF_2_fh_tries,
		tryResult *result, int min, int max, Vec10* stats, int tid);

	IndexThreadReduce<Vec10> makeNewTracesReduce;
	void makeNewTraces_Reductor(FrameHessian* newFrame, std::vector<ImmaturePoint*> *immaturePoints, int min, int max, Vec10* stats, int tid);
	...
	struct trackResult
	{
		...
		int trace_out;
		int trace_skip;
		int trace_badcondition;
		int trace_uninitialized;
	};
	IndexThreadReduce<Vec10> traceReduce;
	void trace_Reductor(const FrameHessian* const fh, FrameHessian* host, trackResult *results, int min, int max, Vec10* stats, int tid);

	boost::mutex toOptimizeMutex;
	IndexThreadReduce<Vec10> toOptimizeReduce;
	void toOptimize_Reductor(FrameHessian* host, std::vector<ImmaturePoint*> *toOptimizes, int min, int max, Vec10* stats, int tid);

	

	
	void createGroundCalibrator();
	void updateGroundCalibrator(bool needToMakeKF, FrameHessian* fh);

	// handle lost tracking
	void updateLostTracker(bool needToMakeKF, FrameHessian* fh);
	bool handleLostTrack();
	void recoverFromLostBackup();

	char fullSystemTrackingState = ' ';
};
}
