...
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
	CoarseInitializer(int channel, int w, int h);
	~CoarseInitializer();
...
private:
	...
	Accumulator9 acc9s[NUM_THREADS];
	Accumulator9 acc9SCs[NUM_THREADS];
	Accumulator11 Es[NUM_THREADS];
	Accumulator11 EAlphas[NUM_THREADS];
	...
	IndexThreadReduce<Vec10> calcResAndGSReduce;
	void calcEAndacc9_Reductor(int lvl, const SE3 &refToNew, AffLight refToNew_aff, int min, int max, Vec10* stats, int tid);
	void calcEAlpha_Reductor(int lvl, int min, int max, Vec10* stats, int tid);
	void calcacc9SC_Reductor(int lvl, float alphaOpt, int min, int max, Vec10* stats, int tid);
};
