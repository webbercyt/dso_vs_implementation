...
struct PointHessian
{
	...
	inline bool isOOB(const std::vector<FrameHessian*>& toKeep, const std::vector<FrameHessian*>& toMarg) const
	{
		...
		{
			if (r->state_state != ResState::ResState_IN) continue;
		...
		if (lastResiduals[0].second == ResState::ResState_OOB) return true;
		if (residuals.size() < 2) return false;
		if (lastResiduals[0].second == ResState::ResState_OUTLIER && lastResiduals[1].second == ResState::ResState_OUTLIER) return true;
		return false;
	}
...
struct FrameHessian
{
	...
	int camFrameID;						// actual camera frame ID
	static int instanceCounter;
	int idx;
	...
	inline ~FrameHessian()
	{
		...
		if(orgImage) delete[] orgImage;
	};
	inline FrameHessian(int channel) :
		channel(channel)
	{
		instanceCounter++;
	...
	inline void setOrgImage(ImageAndExposure* image)
	{
		orgw = image->orgw;
		orgh = image->orgh;
		orgImage = new unsigned char[orgw * orgh];
		memcpy(orgImage, image->orgImage, orgw * orgh);
	}
...
struct CalibHessian
{
	...
	inline CalibHessian(int channel) :
		channel(channel)
	{

		VecC initial_value = VecC::Zero();
		initial_value[0] = globalCalibs[channel].fxG[0];
		initial_value[1] = globalCalibs[channel].fyG[0];
		initial_value[2] = globalCalibs[channel].cxG[0];
		initial_value[3] = globalCalibs[channel].cyG[0];
