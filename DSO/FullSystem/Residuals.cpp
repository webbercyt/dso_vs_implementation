...
PointFrameResidual::PointFrameResidual(int channel) :
	channel(channel)
{assert(false); instanceCounter++;}

PointFrameResidual::~PointFrameResidual(){assert(efResidual==0); instanceCounter--; delete J;}

PointFrameResidual::PointFrameResidual(int channel, PointHessian* point_, FrameHessian* host_, FrameHessian* target_) :
	channel(channel),
	point(point_),
	host(host_),
	target(target_)
...
double PointFrameResidual::linearize(CalibHessian* HCalib)
{
	...
	if(state_state == ResState::ResState_OOB)
		{ state_NewState = ResState::ResState_OOB; return state_energy; }

	FrameFramePrecalc* precalc = &(host->targetPrecalc[target->idx]);
	float energyLeft=0;
	const Eigen::Vector3f* const dIl = target->dI;
	...
	float d_d_x, d_d_y;
	{
		...
		//return PRE_RTll_0.row(0)[0] + PRE_RTll_0.row(0)[1] + PRE_RTll_0.row(0)[2];
		if(!projectPoint(channel, point->u, point->v, point->idepth_zero_scaled, 0, 0,HCalib,
	...
	{
		float Ku, Kv;
		if(!projectPoint(channel, point->u+patternP[idx][0], point->v+patternP[idx][1], point->idepth_scaled, PRE_KRKiTll, PRE_KtTll, Ku, Kv))
			{ state_NewState = ResState::ResState_OOB; return state_energy; }
	...
	{
		state_NewState = ResState::ResState_IN;
	}
...
{
	if(state_state==ResState::ResState_OOB) return;
	...
	{
		if(state_state == ResState::ResState_IN) cT = Vec3b(255,0,0);
		else if(state_state == ResState::ResState_OOB) cT = Vec3b(255,255,0);
		else if(state_state == ResState::ResState_OUTLIER) cT = Vec3b(0,0,255);
		else cT = Vec3b(255,255,255);
	}

	for(int i=0;i<patternNum;i++)
	{
		if((projectedTo[i][0] > 2 && projectedTo[i][1] > 2 && projectedTo[i][0] < globalCalibs[channel].wG[0]-3 && projectedTo[i][1] < globalCalibs[channel].hG[0]-3 ))
...
void PointFrameResidual::applyRes(bool copyJacobians)
{
	if(copyJacobians)
	{
		...
		if(state_NewState == ResState::ResState_IN)// && )
		{
