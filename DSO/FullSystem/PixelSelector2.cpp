...
PixelSelector::PixelSelector(int channel, int w, int h) :
	channel(channel)
{
...
//	else
	...
	if(quotia < 0.95)
	{
		int wh= globalCalibs[channel].wG[0]* globalCalibs[channel].hG[0];
		int rn=0;
		...
		float* map_out, int pot, float thFactor)
{
	...
	int w = globalCalibs[channel].wG[0];
	int w1 = globalCalibs[channel].wG[1];
	int w2 = globalCalibs[channel].wG[2];
	int h = globalCalibs[channel].hG[0];
