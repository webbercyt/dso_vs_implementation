...
CoarseInitializer::CoarseInitializer(int channel, int ww, int hh) : 
	channel(channel),
	thisToNext_aff(0,0), 
	thisToNext(SE3())
{
...
bool CoarseInitializer::trackFrame(FrameHessian* newFrameHessian, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
	...
	for(int lvl=pyrLevelsUsed-1; lvl>=0; lvl--)
	{
		...
		if (setting_useProfile[channel]) {
			...
			printf("\t\tCoarseInitializer::propagateDown(): lvl=%d  %f ms\n", lvl, duration * 1000.0f);
		}
		...
		//printf("CoarseInitializer::resOld=%f,%f,%f\n", resOld[0], resOld[1], resOld[2]);
		applyStep(lvl);

		if (setting_useProfile[channel]) {
			end = clock();
			duration = (double)(end - start) / CLOCKS_PER_SEC;
			printf("\t\tCoarseInitializer::calcResAndGS(): lvl=%d  %f ms\n", lvl, duration * 1000.0f);
		}
		...
		while(true)
		{
			...
			//printf("CoarseInitializer::resNew=%f,%f,%f\n", resNew[0], resNew[1], resNew[2]);
			if (setting_useProfile[channel]) {
				end = clock();
				duration = (double)(end - start) / CLOCKS_PER_SEC;
				printf("\t\t\tCoarseInitializer::calcResAndGS(): lvl=%d  %f ms\n", lvl, duration * 1000.0f);
		...
		if (setting_useProfile[channel]) {
			...
			duration = (double)(end - start) / CLOCKS_PER_SEC;
			printf("\t\tCoarseInitializer::while(true): lvl=%d  %f ms\n\n", lvl, duration * 1000.0f);
		}
	...
	if (setting_useProfile[channel]) {
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		printf("\t\tCoarseInitializer::propagateUp(): %f ms\n", duration * 1000.0f);
	}
	...
	if (setting_useProfile[channel]) {
		...
		duration = (double)(end - start) / CLOCKS_PER_SEC;
		printf("\t\tCoarseInitializer::debugPlot(): %f ms\n", duration * 1000.0f);
	}
...
void CoarseInitializer::debugPlot(int lvl, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
	...
	const Eigen::Vector3f* const colorRef = (Eigen::Vector3f*)firstFrame->dIp[lvl];
...
void CoarseInitializer::calcEAndacc9_Reductor(int lvl,const SE3 &refToNew, AffLight refToNew_aff,
	int min, int max, Vec10* stats, int tid)
{
	int wl = w[lvl], hl = h[lvl];
	const Eigen::Vector3f* const colorRef = (Eigen::Vector3f*)firstFrame->dIp[lvl];
	const Eigen::Vector3f* const colorNew = (Eigen::Vector3f*)newFrame->dIp[lvl];
	...
	for (int k = min; k < max; k++)
	{
		...
		if (!point->isGood)
		{
			Es[tid].updateSingle((float)(point->energy[0]));
		...
		for (int idx = 0; idx<patternNum; idx++)
		{
			...
			//Vec3f hitColor = getInterpolatedElement33BiCub(colorNew, Ku, Kv, wl, hl);
			...
			JbBuffer_new[k][0] += dp0[idx] * dd[idx];
			JbBuffer_new[k][1] += dp1[idx] * dd[idx];
			JbBuffer_new[k][2] += dp2[idx] * dd[idx];
			JbBuffer_new[k][3] += dp3[idx] * dd[idx];
			JbBuffer_new[k][4] += dp4[idx] * dd[idx];
			JbBuffer_new[k][5] += dp5[idx] * dd[idx];
			JbBuffer_new[k][6] += dp6[idx] * dd[idx];
			JbBuffer_new[k][7] += dp7[idx] * dd[idx];
			JbBuffer_new[k][8] += r[idx] * dd[idx];
			JbBuffer_new[k][9] += dd[idx] * dd[idx];
		}
...
void CoarseInitializer::calcEAlpha_Reductor(int lvl, int min, int max, Vec10* stats, int tid)
{
	Pnt* ptsl = points[lvl];
	for (int i = min; i < max; i++)
	{
		...
		if (!point->isGood_new)
		{
			EAlphas[tid].updateSingle((float)(point->energy[1]));
		}
		else
		{
			point->energy_new[1] = (point->idepth_new - 1)*(point->idepth_new - 1);
			EAlphas[tid].updateSingle((float)(point->energy_new[1]));
		}
	}
}

void CoarseInitializer::calcacc9SC_Reductor(int lvl, float alphaOpt, int min, int max, Vec10* stats, int tid)
{
	Pnt* ptsl = points[lvl];
	for (int i = min; i < max; i++)
	{
		...
		acc9SCs[tid].updateSingleWeighted(
		...
		bool plot)
{
	...
	if (multiThreading)
	{
		calcResAndGSReduce.reduce(boost::bind(&CoarseInitializer::calcEAndacc9_Reductor, this, lvl, refToNew, refToNew_aff,
			_1, _2, _3, _4), 0, npts, 0);
	}
	else
	{
		calcEAndacc9_Reductor(lvl, refToNew, refToNew_aff, 0, npts, 0, 0);
		
		/*int wl = w[lvl], hl = h[lvl];
		Eigen::Vector3f* colorRef = (Eigen::Vector3f*)firstFrame->dIp[lvl];
		Eigen::Vector3f* colorNew = (Eigen::Vector3f*)newFrame->dIp[lvl];
	...
	for (int i = 0; i < NUM_THREADS; i++)
		EAlphas[i].initialize();
	if (multiThreading)
	{
		calcResAndGSReduce.reduce(boost::bind(&CoarseInitializer::calcEAlpha_Reductor, this, lvl,
			_1, _2, _3, _4), 0, npts, 0);
	}
	else
	{
		calcEAlpha_Reductor(lvl, 0, npts, 0, 0);
	...
	{
		EAlphas[i].finish();
		EAlpha_A += EAlphas[i].A;
	}
	float alphaEnergy = alphaW*(EAlpha_A + refToNew.translation().squaredNorm() * npts);
	...
	for (int i = 0; i < NUM_THREADS; i++)
		acc9SCs[i].initialize();
	if (multiThreading)
	{
		calcResAndGSReduce.reduce(boost::bind(&CoarseInitializer::calcacc9SC_Reductor, this, lvl, alphaOpt,
			_1, _2, _3, _4), 0, npts, 0);
	}
	else
	{
		calcacc9SC_Reductor(lvl, alphaOpt, 0, npts, 0, 0);
	...
	Mat99f acc9SC_H; acc9SC_H.setZero();
	for (int i = 0; i < NUM_THREADS; i++)
	{
		acc9SCs[i].finish();
		acc9SC_H += acc9SCs[i].H;
	}


	//printf("nelements in H: %d, in E: %d, in Hsc: %d / 9!\n", (int)acc9.num, (int)E.num, (int)acc9SC.num*9);
	H_out = acc9_H.topLeftCorner<8,8>();// / acc9.num;
	b_out = acc9_H.topRightCorner<8,1>();// / acc9.num;
	H_out_sc = acc9SC_H.topLeftCorner<8,8>();// / acc9.num;
	b_out_sc = acc9SC_H.topRightCorner<8,1>();// / acc9.num;
	...
	return Vec3f(E_A, alphaEnergy,E_num);
}
...
void CoarseInitializer::setFirst(	CalibHessian* HCalib, FrameHessian* newFrameHessian)
{
	...
	PixelSelector sel(channel, w[0],h[0]);

	float* statusMap = new float[w[0]*h[0]];
	bool* statusMapB = new bool[w[0]*h[0]];

	float densities[] = {0.03f,0.05f,0.15f,0.5f,1.0f};
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		if(lvl == 0)
			...
			npts = makePixelStatus((Eigen::Vector3f*)firstFrame->dIp[lvl], statusMapB, w[lvl], h[lvl], densities[lvl]*w[0]*h[0]);
		...
		for(int x=patternPadding+1;x<wl-patternPadding-2;x++)
		{
			...
			if((lvl!=0 && statusMapB[x+y*wl]) || (lvl==0 && statusMap[x+y*wl] != 0))
			{
				...
				const Eigen::Vector3f* const cpt = (Eigen::Vector3f*)firstFrame->dIp[lvl] + x + y*w[lvl];
				float sumGrad2=0;
				for(int idx=0;idx<patternNum;idx++)
				{
					int dx = patternP[idx][0];
