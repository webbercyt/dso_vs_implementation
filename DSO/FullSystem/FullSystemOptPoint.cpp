		...
		ImmaturePointTemporaryResidual* residuals)
{
	...
	for(FrameHessian* fh : frameHessians)
	{
		if(fh != point->host)
		{
			...
			residuals[nres].state_NewState = ResState::ResState_OUTLIER;
			residuals[nres].state_state = ResState::ResState_IN;
	...
	int numGoodRes=0;
	for(int i=0;i<nres;i++)
		if(residuals[i].state_state == ResState::ResState_IN) numGoodRes++;
	...
	p->lastResiduals[0].second = ResState::ResState_OOB;
	p->lastResiduals[1].first = 0;
	p->lastResiduals[1].second = ResState::ResState_OOB;
	p->setIdepthZero(currentIdepth);
	p->setIdepth(currentIdepth);
	p->setPointStatus(PointHessian::ACTIVE);

	for(int i=0;i<nres;i++)
		if(residuals[i].state_state == ResState::ResState_IN)
		{
			PointFrameResidual* r = new PointFrameResidual(channel, p, p->host, residuals[i].target);
			r->state_NewEnergy = r->state_energy = 0;
			r->state_NewState = ResState::ResState_OUTLIER;
