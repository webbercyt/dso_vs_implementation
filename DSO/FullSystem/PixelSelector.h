...
template<int pot>
inline int gridMaxSelection(const Eigen::Vector3f* const grads, bool* map_out, int w, int h, float THFac)
{
	...
	for(int y=1;y<h-pot;y+=pot)
	{
		for(int x=1;x<w-pot;x+=pot)
		{
			...
			const Eigen::Vector3f* const grads0 = (Eigen::Vector3f*)grads+x+y*w;
...
inline int gridMaxSelection(const Eigen::Vector3f* const grads, bool* map_out, int w, int h, int pot, float THFac)
{
	...
	for(int y=1;y<h-pot;y+=pot)
	{
		for(int x=1;x<w-pot;x+=pot)
		{
			...
			Eigen::Vector3f* grads0 = (Eigen::Vector3f*)grads+x+y*w;
			for(int dx=0;dx<pot;dx++)
				for(int dy=0;dy<pot;dy++)
				{
					int idx = dx+dy*w;
...
inline int makePixelStatus(const Eigen::Vector3f* const grads, bool* map, int w, int h, float desiredDensity, int recsLeft=5, float THFac = 1)
{
