...
public:
	...
	inline ImageAndExposure* getDeepCopy()
	{
		...
		img->camFrameID = camFrameID;
		if (orgImage)
		{
			img->orgImage = new unsigned char[orgw * orgh];
			memcpy(img->orgImage, orgImage, w*h * sizeof(unsigned char));
		}
		memcpy(img->image, image, w*h*sizeof(float));
		return img;
	}

	inline void setOrgImage(MinimalImageB* minimg)
	{
		orgw = minimg->w;
		orgh = minimg->h;
		orgImage = new unsigned char[orgw * orgh];
		memcpy(orgImage, minimg->data, orgw * orgh);
	}
};
