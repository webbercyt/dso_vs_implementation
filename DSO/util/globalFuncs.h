...
EIGEN_ALWAYS_INLINE int clamp(const float val, const int min, const int max)
{
	int ix = (int)val;
	if (ix < min) {
		ix = min;
		printf("Out range detected: val=%f\n", val);
	}
	if (ix >= max - 1) {
		ix = max - 2;
		printf("Out range detected: val=%f\n", val);
...
EIGEN_ALWAYS_INLINE float getInterpolatedElement(const float* const mat, const float x, const float y, const int width, const int height)
{
	//stats.num_pixelInterpolations++;

	int ix = clamp(x, 0, width);
	int iy = clamp(y, 0, height);
	float dx = x - ix;
	float dy = y - iy;
	float dxdy = dx*dy;
...
EIGEN_ALWAYS_INLINE Eigen::Vector3f getInterpolatedElement43(const Eigen::Vector4f* const mat, const float x, const float y, const int width, const int height)
{
	int ix = clamp(x, 0, width);
	int iy = clamp(y, 0, height);
	float dx = x - ix;
...
EIGEN_ALWAYS_INLINE Eigen::Vector3f getInterpolatedElement33OverAnd(const Eigen::Vector3f* const mat, const bool* overMat, const float x, const float y, const int width, const int height, bool& over_out)
{
	int ix = clamp(x, 0, width);
	int iy = clamp(y, 0, height);
	float dx = x - ix;
...
EIGEN_ALWAYS_INLINE Eigen::Vector3f getInterpolatedElement33OverOr(const Eigen::Vector3f* const mat, const bool* overMat, const float x, const float y, const int width, const int height, bool& over_out)
{
	int ix = clamp(x, 0, width);
	int iy = clamp(y, 0, height);
	float dx = x - ix;
...
EIGEN_ALWAYS_INLINE Eigen::Vector3f getInterpolatedElement13BiLin(const float* const mat, const float x, const float y, const int width, const int height)
{
	int ix = clamp(x, 0, width);
	int iy = clamp(y, 0, height);
	const float* bp = mat +ix+iy*width;
...
EIGEN_ALWAYS_INLINE Eigen::Vector3f getInterpolatedElement33BiLin(const Eigen::Vector3f* const mat, const float x, const float y, const int width, const int height)
{
	int ix = clamp(x, 0, width);
	int iy = clamp(y, 0, height);
	const Eigen::Vector3f* bp = mat +ix+iy*width;
...
EIGEN_ALWAYS_INLINE float getInterpolatedElement11BiCub(const float* const mat, const float x, const float y, const int width, const int height)
{
	int ix = clamp(x, 0, width);
	int iy = clamp(y, 0, height);
	float dx = x - ix;
...
EIGEN_ALWAYS_INLINE Eigen::Vector3f getInterpolatedElement13BiCub(const float* const mat, const float x, const float y, const int width, const int height)
{
	int ix = clamp(x, 0, width);
	int iy = clamp(y, 0, height);
	float dx = x - ix;
...
EIGEN_ALWAYS_INLINE Eigen::Vector3f getInterpolatedElement33BiCub(const Eigen::Vector3f* const mat, const float x, const float y, const int width, const int height)
{
	int ix = clamp(x, 0, width);
	int iy = clamp(y, 0, height);
	float dx = x - ix;
...
EIGEN_ALWAYS_INLINE Eigen::Vector4f getInterpolatedElement44(const Eigen::Vector4f* const mat, const float x, const float y, const int width, const int height)
{
	int ix = clamp(x, 0, width);
	int iy = clamp(y, 0, height);
	float dx = x - ix;
...
EIGEN_ALWAYS_INLINE Eigen::Vector2f getInterpolatedElement42(const Eigen::Vector4f* const mat, const float x, const float y, const int width, const int height)
{
	int ix = clamp(x, 0, width);
	int iy = clamp(y, 0, height);
	float dx = x - ix;
