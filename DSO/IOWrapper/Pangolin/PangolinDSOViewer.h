...
struct FrameShell;
	...
	namespace IOWrap
	{
		...
		public:
			...
			PangolinDSOViewer(int channel, int w, int h, bool startRunThread=true);
		...
		private:
			...
			std::vector<KeyFrameDisplay*> allFrames;
			...
			float delta_x, delta_y, delta_z, delta_scale;
			void calcDelta();
			void focusCamera();

			// point cloud settings
			bool settings_pointCloudAlign;
			bool settings_pointCloudSave;

			bool settings_groundPoseCalib;

			// render settings
			bool settings_showFollowCam;
			...
			IndexThreadReduce<Vec10> imageReduce;
			void image_Reductor(const FrameHessian* const fh, MinimalImageB3 *img, int min, int max, Vec10* stats, int tid);
